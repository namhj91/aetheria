        ...
        function renderHistoryLayout() {
            ...
            if (minimapToggleBtn) {
                minimapToggleBtn.onclick = () => {
                    ...
                    if (minimapBody) minimapBody.className = state.minimapCollapsed ? 'p-2 hidden' : 'p-2';
                    minimapToggleBtn.textContent = state.minimapCollapsed ? '>' : 'v';
                    if (typeof scheduleMiniMapUpdate === 'function') scheduleMiniMapUpdate(true);
                };
            }
            if (typeof makeDockable === 'function') makeDockable('minimap-panel', 'minimap-header');
            renderHistoryUI();
        }
